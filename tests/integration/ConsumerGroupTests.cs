...
///
/// detect consumes <c>my-topic</c> (group "detect"); ledger consumes
...
public class ConsumerGroupTests(StackFixture fixture)
{
    ...
    [Fact]
    public async Task Detect_Consumes_FromMyTopic()
    {
        await KafkaTestClient.ProduceAsync(fixture.Bootstrap, "my-topic",
            JsonSerializer.Serialize(new { marker = Guid.NewGuid().ToString("N") }));

        var caughtUp = await KafkaTestClient.WaitForGroupCaughtUpAsync(
            fixture.Bootstrap, "detect", "my-topic", Timeout);

        Assert.True(caughtUp, "detect consumer group did not catch up to my-topic");
    }
    ...
    public async Task Ledger_Consumes_FromCategorizedTopic()
    {
        ...
        var caughtUp = await KafkaTestClient.WaitForGroupCaughtUpAsync(
            fixture.Bootstrap, "my-group", "transaction.categorized", Timeout);
